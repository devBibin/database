...
def get_person_by_id(db, cursor, person_id):
    cursor.execute("""select name,second_name,surname,street,house,building,ph_home,ph_mobile,ph_work,email from person where id=(?)""",
     (person_id,)
    )
...
def get_target_contribution_by_id(db, cursor, person_id):
    cursor.execute("""select name, sum from target_contribution where person_id=(?)""",
     (person_id,)
    )
    return cursor.fetchall()
...
@db_decorator
def delete_debts_by_id(db, cursor, person_id):
    cursor.execute("""delete from debts where person_id =(?)""",
     (person_id,)
    )
    db.commit()
    return cursor.rowcount
